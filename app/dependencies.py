...
def require_face_api_key(x_api_key: str | None = Header(default=None, alias="X-API-Key")) -> None:
    ...
    if not configured:
        raise HTTPException(status_code=503, detail="Khóa API khuôn mặt chưa được cấu hình trên máy chủ.")
    if x_api_key is None or not hmac.compare_digest(
        x_api_key.encode("utf-8"), configured.encode("utf-8")
    ):
        raise HTTPException(status_code=401, detail="Khóa API không hợp lệ. Vui lòng kiểm tra lại giá trị X-API-Key.")
